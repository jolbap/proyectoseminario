...
public class MapsBorrarVecindario extends FragmentActivity implements OnMapReadyCallback, GoogleMap.OnPolygonClickListener, View.OnClickListener{
    ...
    private Button btnborrarvecindario;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        ParamsConnection.LISTDATA = new ArrayList<ItemList>();
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_maps_borrar_vecindario);
        intent = getIntent();
        positions = new ArrayList<>();
        idV = intent.getExtras().getString("idVecindario");
        // Obtain the SupportMapFragment and get notified when the map is ready to be used.
        SupportMapFragment mapFragment = (SupportMapFragment) getSupportFragmentManager()
                .findFragmentById(R.id.vecindario_detalle_borrar);
        mapFragment.getMapAsync(this);
        vecindarionom = this.findViewById(R.id.vecinnombre);
        btnborrarvecindario = (Button)findViewById(R.id.btnborrarvecindario);
        btnborrarvecindario.setOnClickListener(this);
        loadVecindario();
    }
    ...
    public void onClick(View v) {
            AsyncHttpClient client = new AsyncHttpClient();
            String url = ParamsConnection.HOST_VECINDARIO + idV;
            client.delete(url, new JsonHttpResponseHandler(){

            });
            Intent intent = new Intent(this, FuncionesAgente.class);
            Toast.makeText(getApplicationContext(),"Vecindario Borrado",Toast.LENGTH_LONG).show();
